...
- (void)network {
    [HttpClient getMallOrderOfBuyWithStatus:nil page:@1 WithBlock:^(NSDictionary *dictionary) {
        ...
        if (statusCode == 200) {
            ...
            if (self.mallBuyHistoryArray.count == 0) {
                mallBuyTableView.backgroundView = [[TableViewHeaderView alloc] initWithFrame:CGRectMake(0, 0, kScreenW, kScreenH - 64 - 44) withImage:@"数据暂无" withLabelText:@"暂无购买记录"];
            } else {
...
- (void)footerRereshing{
    ...
    [HttpClient getMallOrderOfBuyWithStatus:status page:@(refrushCount) WithBlock:^(NSDictionary *dictionary) {
        NSInteger statusCode = [dictionary[@"statusCode"] integerValue];
        if (statusCode == 200) {
            for (NSMutableDictionary *historyDic in dictionary[@"data"]) {
                MeHistoryOrderModel *historyOrderModel = [MeHistoryOrderModel getMeHistoryOrderModelWithDictionary:historyDic];
    ...
    [HttpClient getMallOrderOfBuyWithStatus:status page:@(refrushCount) WithBlock:^(NSDictionary *dictionary) {
        ...
        if (statusCode == 200) {
            ...
            if (self.mallBuyHistoryArray.count == 0) {
                ...
                mallBuyTableView.backgroundView = [[TableViewHeaderView alloc] initWithFrame:CGRectMake(0, 0, kScreenW, kScreenH - 64 - 44) withImage:@"数据暂无" withLabelText:@"暂无交易记录"];
            } else {
                mallBuyTableView.backgroundView = nil;
                [self setupRefresh];
            }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    DLog(@"订单详情");
    MallHistoryDetail_VC *mallDetailVC = [[MallHistoryDetail_VC alloc] initWithStyle:UITableViewStyleGrouped];
...
- (void)actionOfStatus:(UIButton *)button {
    ...
    if ([button.titleLabel.text isEqualToString:@"付款"]) {
        ...
        //解析工厂信息
        [HttpClient getOtherIndevidualsInformationWithUserID:historyOrder.userId WithCompletionBlock:^(NSDictionary *dictionary) {
            ...
            conversationVC.targetId = historyOrder.userId; // 接收者的 targetId，这里为举例。
