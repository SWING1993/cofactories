...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.newsTitle = [[UILabel alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(self.photoView.frame), self.frame.size.width, 30*kZGY)];
        self.newsTitle.numberOfLines = 2;
        self.newsTitle.font = [UIFont systemFontOfSize:12*kZGY];
        [self addSubview:self.newsTitle];
        self.commentCountLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(self.newsTitle.frame), self.frame.size.width, 20*kZGY)];
