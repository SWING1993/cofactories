...
@implementation OrderDetail_Fac_TVC{
    UILabel *_orderIdlB;
...
- (void)setupUI{
    ...
    for (int i = 0; i<6; i++) {
        ...
        label.text = @[@"类型",@"数量",@"下单时间",@"交货日期",@"担保金额",@"备注信息"][i];
        [self.contentView addSubview:label];
...
- (void)setModel:(FactoryOrderMOdel *)model{
    ...
    if (_isRestrict) {
        _orderMoneyLB.frame = CGRectMake(90, 40+100, kScreenW-90, 25);
        _commentLB.frame = CGRectMake(90, 40+125+5, textSize.width, textSize.height);

    }else{
        [_orderMoneyLB removeFromSuperview];
        [_orderMoneyTitle removeFromSuperview];
        
        _commentTitle.frame = CGRectMake(15, 40+100, 60, 25);
        _commentLB.frame = CGRectMake(90, 40+100+5, textSize.width, textSize.height);
    }
}
